...
public class AdminEmployeeController {
    ...
    private final DepartmentService deptS;
    private final UserRepository userRepo;

    private void base(Model m) {
        m.addAttribute("departments", deptS.findAllSimple());
        m.addAttribute("activePage", "employees");
    }

    @GetMapping
    public String list(@RequestParam(required=false) String q, Model m) {
        m.addAttribute("employees", empS.search(q));
    ...
    public String save(@ModelAttribute Employee emp,
        ...
        if (empS.emailExists(emp.getEmail())) {
    ...
    public String edit(@PathVariable Long id, Model m) {
        m.addAttribute("employee", empS.findById(id));
        base(m);
        return "admin/employees/form";
    }
    ...
    public String update(@PathVariable Long id,
        ...
        if (!ex.getEmail().equals(emp.getEmail()) && empS.emailExistsForOther(emp.getEmail(), id)) {
            ra.addFlashAttribute("error", "Email already in use");
            return "redirect:/admin/employees/" + id + "/edit";
        }
        if (departmentId != null) emp.setDepartment(deptS.findById(departmentId));
        emp.setId(id);
        emp.setUser(ex.getUser());
        empS.save(emp);
