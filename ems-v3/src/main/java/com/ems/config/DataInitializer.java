...
public class DataInitializer implements CommandLineRunner {
    ...
    @Override public void run(String... args) {
        if(!userRepo.existsByUsername("admin")){
            userRepo.save(User.builder().username("admin").password(enc.encode("admin123")).role(Role.ROLE_ADMIN).build());
        ...
        if(!userRepo.existsByUsername("mohan")){
            ...
            var u=userRepo.save(User.builder().username("mohan").password(enc.encode("mohan123")).role(Role.ROLE_EMPLOYEE).build());
            empRepo.save(Employee.builder().firstName("Mohan").lastName("Kumar").email("mohan@company.com").phone("9876543210")
                .department(dept).position("Software Engineer").joinDate(LocalDate.of(2024,1,15))
                .basicSalary(55000.0).gender("Male").status(Employee.EmpStatus.ACTIVE).user(u).build());
            System.out.println("[EMS] Employee created — mohan / mohan123");
        }
        if(annRepo.count()==0){
            annRepo.save(Announcement.builder().title("Welcome to EMS v3!")
                .message("Employee Management System is live. Thymeleaf + JWT (30-min) edition.")
                .postedBy("Admin").audience(Announcement.Audience.ALL_STAFF)
                .priority(Announcement.Priority.IMPORTANT).postDate(LocalDate.now()).build());
        }
    }
}